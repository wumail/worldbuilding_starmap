...
def analyze_disk_concentration(stars):
    ...
    print("="*60)
    print(f"{'距离区间 (pc)':<20} | {'总数':<8} | {'盘面占比 (±10°)':<18} | {'集中倍率':<10}")
    print("-" * 65)
    ...
    for label, data in bins.items():
        ...
        # 集中倍率评价
        eval_str = ""
        if factor < 1.3: eval_str = "(球状分布)"
        elif factor > 2.0: eval_str = "(显著盘状!)"
        
        print(f"{label:<20} | {count:<8} | {ratio:.1%}             | {factor:.1f}x {eval_str}")
    
    print("-" * 65)
    print(f"* 理论各向同性占比: {ISOTROPIC_REF:.1%}")
    print(f"* 解释: 远景恒星的集中倍率越高，说明银河带越明显。")
