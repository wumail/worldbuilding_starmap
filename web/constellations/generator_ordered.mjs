...
import {generateDraw as generateLegacyDraw,normalizeRecipe as normalizeLegacyRecipe,validateDraw as validateLegacyDraw,ALGORITHM as LEGACY_ALGORITHM} from './generator_legacy.mjs';
import {LAYOUT_RULES,layoutIssues} from './region_layout.mjs';
...
export const ALGORITHM = 'terrax-zodiac-draw-2';
export const SUPPORTED_ALGORITHMS = Object.freeze([ALGORITHM,LEGACY_ALGORITHM]);
export const RULES = Object.freeze({count:15, candidateMagnitude:4.5, preferredMagnitude:4, searchLatitude:30, siteLatitude:LAYOUT_RULES.siteLatitude, displayScale:2.5, maxEdge:13, minSeparation:.8, maxDiameter:42, maxDegree:4, lineClearance:.18, outerSiteLatitude:50, boundaryClearance:2});
...
export function normalizeRecipe(value={}) {
    if(value.algorithm===LEGACY_ALGORITHM)return normalizeLegacyRecipe(value);
...
function partition(pool,seed) {
    ...
    for(let attempt=0;attempt<24;attempt++){
        ...
        let centers=Array.from({length:15},(_,i)=>vector(i*24+phase+(rng()-.5)*10,(rng()-.5)*RULES.siteLatitude*2));
        for(let iteration=0;iteration<75;iteration++){
            ...
            const next=sums.map((sum,i)=>{if(Math.hypot(...sum)<1e-12)return centers[i];const c=coordinates(sum);return vector(c.longitude,Math.max(-RULES.siteLatitude,Math.min(RULES.siteLatitude,c.latitude)));});
        ...
        const allGroups=centers.map(()=>[]);for(const s of pool)allGroups[owner(s.direction,centers)].push(s);
        // Keep figure members inside their own interval along the ecliptic as
        // well as their actual 2D cell. Small border bends may overlap in x;
        // the recognizable figures themselves cannot stack north/south in x.
        const groups=allGroups.map((group,i)=>group.filter(s=>owner(vector(s.longitude,0),centers)===i));
        ...
        for(const sign of [-1,1])for(let longitude=12;longitude<360;longitude+=24){
            ...
            for(let latitude=RULES.outerSiteLatitude;latitude<=90;latitude++){
                ...
                if(allGroups.every((g,i)=>g.every(s=>separation(s.direction,site)>=separation(s.direction,centers[i])+RULES.boundaryClearance)))break;
            }
            remainderSites.push(site);
        }
        const all=[...centers,...remainderSites];
        if(allGroups.some((g,i)=>g.some(s=>owner(s.direction,all)!==i)))continue;
        const sections=eclipticIntervals(all);
        if(sections.some(i=>i.index>=15))continue;
        const boundaries=centers.map((site,i)=>({id:`Z${String(i+1).padStart(2,'0')}`,site,polygon:cellPolygon(i,all),intervals:sections.filter(s=>s.index===i)}));
        if(layoutIssues(boundaries).length)continue;
...
export function generateDraw(source,meta,value={}) {
    if(value.algorithm===LEGACY_ALGORITHM)return generateLegacyDraw(source,meta,value);
    ...
    const result={schema:3,status:'candidate-review',catalogue:meta.catalogue,sha256:meta.sha256,epoch:'Terrax 第 0 日参考黄道',recipe,settings:RULES,layoutRules:LAYOUT_RULES,sourceCount:stars.length,candidateCount:pool.length,eligibleCandidateCount:p.groups.reduce((n,g)=>n+g.length,0),
...
export function validateDraw(data,stars) {
    if(data.recipe?.algorithm===LEGACY_ALGORITHM)return validateLegacyDraw(data,stars);
    ...
    const layout=layoutIssues(data.regions);if(layout.length)fail(`天区排列：${layout[0]}`);
    const pool=stars.filter(s=>s.app_mag<=RULES.candidateMagnitude&&Math.abs(s.latitude)<=RULES.searchLatitude),eligible=sites.slice(0,15).map(()=>[]);
    for(const s of pool){const i=owner(s.direction,sites);if(i<15&&owner(vector(s.longitude,0),sites)===i)eligible[i].push(s);}
    if(data.eligibleCandidateCount!==eligible.reduce((n,g)=>n+g.length,0))fail('可选恒星总数');
    for(const [i,r] of data.regions.entries()){
        if(r.candidateCount!==eligible[i].length||r.brightCount!==eligible[i].filter(s=>s.app_mag<=4).length)fail('本区选星池');
        ...
        for(const s of r.members){
            ...
            used.add(s.id);
            if(s.app_mag>RULES.candidateMagnitude||Math.abs(s.latitude)>RULES.searchLatitude||owner(s.direction,sites)!==i||owner(vector(s.longitude,0),sites)!==i)fail('成员亮度或区域归属');
        }
