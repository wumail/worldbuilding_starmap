import {generateSampledReference as fittedDraw,ENVELOPE_POLICY} from './generator_fitted.mjs?revision=candidate-editor-band-1';
...
import {normalizeSampling,samplingInfo} from './sampling.mjs';
export {RULES,DEFAULT_SEED,prepareMinimumSolver};
export const ALGORITHM='terrax-zodiac-draw-9';
...
export function normalizeRecipe(value={}){
    ...
    return {...normalizeFree({...value,algorithm:'terrax-zodiac-draw-3'}),algorithm:ALGORITHM,samplingHalfWidthDegrees:normalizeSampling(value.samplingHalfWidthDegrees)};
}
...
function reference(source,meta,seed,halfWidth,onProgress){
    ...
    for(let attempt=0;attempt<12;attempt++){
        ...
        try{
            const old=fittedDraw(stars,meta,{seed:layoutSeed,style:'balanced'},halfWidth),cells=unpackGrid(old.territories),groups=regionalStars(stars,cells);
...
export function generateDraw(source,meta,value={},options={}){
    const recipe=normalizeRecipe(value),onProgress=options.onProgress??(()=>{}),base=reference(source,meta,recipe.seed,recipe.samplingHalfWidthDegrees,onProgress);
    ...
    const data=rebuildRegions({...base,schema:11,sampling:samplingInfo(source,recipe.samplingHalfWidthDegrees),catalogue:meta.catalogue,sha256:meta.sha256,regions,brightPolicy:BRIGHT_POLICY,localPolicy:LOCAL_POLICY},source,cells,recipe);
...
export function validateDraw(data,source){
    ...
    const expected=reference(source,{catalogue:data.catalogue,sha256:data.sha256},recipe.seed,recipe.samplingHalfWidthDegrees,()=>{});
