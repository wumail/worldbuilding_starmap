...
星座工作台当前使用 **draw-10**：种子生成、抽下一轮和本区重生成共用清晰度算法，默认 **标准**，另有简洁和丰富；重要亮星始终保留，减少尖角、密集分叉和冗余闭环。初始采样带默认 ±40°，可调 15°–60°。参考布局及正交划界沿用 draw-9，再在每个实际区域内重新选择、连接最终星形；保留支撑边界的轮廓星与必要连接星。[当前方法与对照](../design/zodiac_figure_clarity.md)说明复杂度软目标和验证范围。

候选窗口提供拖边、增删成员与连线、视野拖动缩放，支持同一草稿切换工具、撤销重做、确认应用与取消恢复。本区复杂度独立设置，默认标准；旧线默认隐藏，可打开比较。本地列表和存储仅保留收藏，新结果需显式收藏才会在刷新后恢复。manual-3 解除自动选星、亮星必选、余量和黄道宽度等限制，只保留源星真实性、正交唯一归属及完整包围检查，不继承自动最优证明。旧 draw-1 至 draw-9 和 manual-1/2 均按原规则恢复；正式 V1/V2 仍使用原有十五个等宽天区。[收藏与本区重新生成](../design/zodiac_regional_sampling.md)及[当前手动操作](../design/zodiac_manual_editing.md)说明使用方式，[自动采样](../design/zodiac_candidate_editing.md)保留滑块和历史生成方法。
