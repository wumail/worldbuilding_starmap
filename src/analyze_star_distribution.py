...
def analyze_star_distribution(json_path=None):
    """分析星星视星等分布"""
    ...
    if json_path is None:
        ...
        if not files:
            raise FileNotFoundError("没有星表，请先生成数据或指定文件")
        json_path = files[0]
    ...
    print("=" * 50)
    print("星图数据统计报告（参考比例不作为通过条件）")
    print("=" * 50)
    print(f"背景星 (stars): {len(stars)}")
    print(f"总计: {len(all_stars)}")
    print()

    # 沿用旧报告以 x.5 为边界的比较区间；参考比例不构成物理约束。
    STANDARD_BINS = [
        (-1.5, -0.5, 0.02),
        (-0.5, 0.5, 0.07),
        (0.5, 1.5, 0.16),
    ...
    print("视星等分布 (旧报告比较区间)")
    print(f"{'=' * 60}")
    print(f"{'区间':<15} {'数量':>8} {'实际%':>10} {'旧模型参考%':>10} {'偏差':>10}")
    print(f"{'-' * 60}")
    ...
    print(f"比较区间以外: {total - sum(bin_counts)} 颗（更亮的星和 6.5 等及以上均计入完整统计）")
