...
import {POINT_STYLE,SYMBOL_REFERENCE,DEFAULT_DISPLAY_SCALE,displaySky as chartSky,pointRadius,pointOpacity,unresolvedPointWeight,solarAtlasCircles} from '../shared/sky_render.mjs';
...
uniform vec3 solarNorth,solarSouth;
uniform float solarDistance;
uniform bool solarMap;
...
vec3 solarCircleAt(vec2 pixel){return length(pixel-atlasNorth.xy)<=atlasNorth.z?solarNorth:solarSouth;}
bool behindSolarLimb(vec2 pixel,float distance){
    vec3 circle=solarCircleAt(pixel);
    return solarMap && distance>solarDistance && circle.z>0. && length(pixel-circle.xy)<circle.z;
}
...
uniform bool luminous,roundSolar;
vec4 sampleBody(vec2 pixel){
    ...
    vec2 uv;
    if(roundSolar){
        vec3 circle=solarCircleAt(pixel);if(circle.z<=0.)return vec4(0.);
    ...
    float shade=luminous?1.:max(0.,dot(vec3(uv,sqrt(max(0.,1.-dot(uv,uv)))),light));
...
void main(){
    ...
    vec3 circle=solarCircleAt(gl_FragCoord.xy);
    bool interior=roundSolar?length(gl_FragCoord.xy-circle.xy)<circle.z-2.5:angle<angularRadius-2.5/focal;
...
void main(){
 ...
 if(mapSymbols || pointSymbolMix>0.){vec3 base=displaySky(rayAt(gl_FragCoord.xy));float coverage=0.;
  for(int y=0;y<4;y++)for(int x=0;x<4;x++)if(!behindSolarLimb(gl_FragCoord.xy+(vec2(float(x),float(y))+.5)/4.-.5,drawWeights.w))coverage+=1./16.;
...
function uniforms() {
    return {...diffuseUniforms(),deepSky:{value:true},displayPass:{value:false},white:{value:false},adaptation:{value:.005},exposureEV:{value:0},displayKey:{value:.002},bounds:{value:new THREE.Vector4(-1,-1,1,1)},resolution:{value:new THREE.Vector2(1,1)},focal:{value:1},projectionKind:{value:0},
        ...
        solarMap:{value:false},solarNorth:{value:new THREE.Vector3(0,0,-1)},solarSouth:{value:new THREE.Vector3(0,0,-1)},solarDistance:{value:0},
...
export class EyeSkyRenderer {
    constructor(canvas) {
        ...
        this.body=material(quadVertex,bodyFragment,{...this.u,bodyDirection:{value:new THREE.Vector3()},axisX:{value:new THREE.Vector3()},axisY:{value:new THREE.Vector3()},light:{value:new THREE.Vector3()},tint:{value:new THREE.Vector3()},angularRadius:{value:0},peak:{value:0},sourceWeight:{value:1},luminous:{value:false},roundSolar:{value:false}},{transparent:true});
    ...
    addPoint(direction,flux,hex,{rawWeight=1,displayWeight=1,distance=1e30}={}) {
        ...
        for(let index=0;index<hemispheres.length;index++){
            ...
            if(this.u.solarMap.value && distance===1e30){
                const limb=(north?this.u.solarNorth:this.u.solarSouth).value;
                // An occulted background point emits no visible PSF at all;
                // clipping its already-spread wings alone would leave a halo.
                if(limb.z>0 && Math.hypot(p.x*this.dpr-limb.x,(this.height-p.y)*this.dpr-limb.y)<=limb.z)continue;
            }
    ...
    boundsForBody(body) {
        if(this.overview && this.u.displayPass.value && body.id==='Sol'){
            const circles=this.solarCircles(body);if(!circles.length)return null;
    ...
    solarCircles(body) {return solarAtlasCircles(body,this.layout,this.orientation).filter(p=>!this.sky.frame.surface || p.north);}
    ...
    render(camera,sky,stars,options={}) {
        ...
        const scene=(sceneSky,target,displayPass)=>{
          ...
          const mapDisplay=displayPass && this.overview,solar=sceneSky.bodies.find(b=>b.id==='Sol');
          u.solarMap.value=!!(mapDisplay && solar);u.solarNorth.value.set(0,0,-1);u.solarSouth.value.set(0,0,-1);
          if(u.solarMap.value){u.solarDistance.value=solar.distance;for(const p of this.solarCircles(solar))(p.north?u.solarNorth:u.solarSouth).value.set(p.x*this.dpr,(this.height-p.y)*this.dpr,p.radius*this.dpr);}
          // The circular Sun is masked per display pixel. Retain angular
          // occultation for all other bodies and for the physical scene.
          const angularBodies=mapDisplay?sceneSky.bodies.filter(b=>b.id!=='Sol'):sceneSky.bodies;
          ...
          for(const body of [...sceneSky.bodies].sort((a,b)=>b.distance-a.distance)) {
            if((sceneSky.frame.surface && body.altitude < -body.angularDiameter/2 && !(mapDisplay && body.id==='Sol')) || (!mapDisplay && bodyFullyOcculted(body,sceneSky)))continue;
            ...
            b.tint.value.copy(luminanceTint(options.white?'#ffffff':body.color));b.luminous.value=body.id==='Sol';b.roundSolar.value=mapDisplay && body.id==='Sol';b.sourceWeight.value=displayPass?1-pointWeight:1;this.pass(this.body,target);
          }
        };
        scene(sky,raw,false);
        ...
        if(this.presentation){
            ...
            u.solarNorth.value.multiplyScalar(rasterScale);u.solarSouth.value.multiplyScalar(rasterScale);
        ...
        if(this.presentation){
            ...
            u.solarNorth.value.multiplyScalar(1/rasterScale);u.solarSouth.value.multiplyScalar(1/rasterScale);
        }
