...
// A map symbol for the Sun: retain the projected center and local disk area,
// but remove the azimuthal map's radial/tangential aspect-ratio distortion.
// Both atlas renderers, glare and display masks use this same circular limb.
export function solarAtlasCircles(body,layout,orientation,angularRadius=body.angularDiameter*DEG/2) {
    ...
    return [true,false].map(north=>{
        ...
        return {...p,north,radius,visible:Math.hypot(p.x-center[0],p.y-center[1])<=layout.radius+radius};
    }).filter(p=>p.visible);
}
