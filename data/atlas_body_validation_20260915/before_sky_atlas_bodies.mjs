import {DEG,unit,add,scale,projectHemisphere} from '../shared/solar_system.mjs';
import {SYMBOL_REFERENCE,diskBasis,diskLight,phaseImage,unresolvedPointWeight,skyBackground,SOLAR_GLARE_RADIUS_RATIO,bodyGlareImage,solarGlareVisibility,solarAtlasCircles,lunarSurfaceScale} from '../shared/sky_render.mjs';
...
export class AtlasBodyPainter {
    ...
    drawSky(sky,l,orientation,{solarGlow=false,...options}={}) {
        ...
        for(const body of [...sky.bodies].sort((a,b)=>b.distance-a.distance)) {
            ...
            // not a valid early-out for the Sun's circular map symbol.
    ...
    disk(body,l,orientation,sky,texture,angularRadius,opacity=1) {
        const ctx=this.ctx,basis=diskBasis(body.view),epsilon=1e-5;
        if(body.id==='Sol') {
            for(const p of solarAtlasCircles(body,l,orientation,angularRadius)) {
                ...
                ctx.transform(p.radius,0,0,p.radius,p.x,p.y);this.paintTexture(texture);ctx.restore();
            }
            return;
        }
        for(const north of [true,false]) {
            if(sky.frame.surface && !north)continue;
            if((north?body.view[2]:-body.view[2]) < -Math.sin(angularRadius))continue;
            const p=projectHemisphere(body.view,l.radius,l,orientation,north);
            const px=projectHemisphere(unit(add(body.view,scale(basis.x,epsilon))),l.radius,l,orientation,north);
            const py=projectHemisphere(unit(add(body.view,scale(basis.y,epsilon))),l.radius,l,orientation,north);
            const factor=Math.tan(angularRadius)/epsilon,c=north?l.north:l.south;
            ctx.save();ctx.beginPath();ctx.arc(...c,l.radius,0,Math.PI*2);ctx.clip();ctx.globalAlpha*=opacity;
            ctx.transform((px.x-p.x)*factor,(px.y-p.y)*factor,-(py.x-p.x)*factor,-(py.y-p.y)*factor,p.x,p.y);
